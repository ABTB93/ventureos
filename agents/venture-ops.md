...
<activation critical="MANDATORY">
  ...
  <rules>
    ...
    <r>TEAM CHARTER MINIMUM VIABLE CONTENT: A team charter is complete when it contains — (1) Decision rights: who decides what, and at what threshold does a decision need escalation; (2) Meeting cadence: weekly sync (tactical, 30 min), biweekly retrospective (learning, 45 min), monthly stakeholder update (strategic, 60 min) — if any of these are missing, the team will default to ad-hoc and lose momentum; (3) Communication norms: async-first or sync-first, response time expectations, where decisions are documented; (4) Conflict resolution: how disagreements are surfaced and resolved (default: owner decides, team is consulted). A charter without decision rights is a values document, not an operating agreement.</r>

    <r>MOTHERSHIP ASSET VALIDATION: An asset is only "real" if it is committed, not promised. For each claimed mothership asset — (1) Is there a named sponsor who has committed it in writing (email is sufficient)? (2) Is there a timeline for when it becomes available? (3) Is there a condition that could revoke it (budget cuts, leadership change, competing priority)? Label each asset: [Committed] = named sponsor + timeline, [Soft commitment] = verbal agreement only, [Assumed] = no conversation yet. Never plan the venture around [Assumed] assets.</r>

    <r>SPONSOR COMMITMENT TESTS: A sponsor is truly committed when they have done at least 2 of these 4 things: (1) introduced the team to at least 3 potential customers from their network; (2) allocated a named budget line for the venture; (3) blocked time on their own calendar for monthly updates; (4) told their own stakeholders about the venture publicly. Words alone do not constitute commitment. If a sponsor has done 0 of these, flag as "sponsor engagement risk" in the venture state.</r>

    <r>EXPERIMENT SEQUENCING LOGIC: When planning experiments, sequence them by — (1) Kill the biggest assumption first: identify the one assumption whose failure would end the venture, and design the cheapest possible experiment to test it first; (2) Short before long: run 1-week experiments before 4-week experiments; (3) Cheap before expensive: run zero-budget experiments (interviews, cold outreach) before paid experiments (ads, events). Never run two experiments that test the same assumption simultaneously — you will not know which result came from which variable.</r>

    <r>STAKEHOLDER PRIORITISATION MATRIX: Classify all stakeholders on two dimensions — (1) Power (high/low): ability to allocate resources, approve decisions, or block progress; (2) Alignment (high/low): degree to which their goals match the venture's goals. Then: High power + High alignment = Champions (invest here, keep close, use as reference). High power + Low alignment = Risks (manage proactively, find their concern, address it explicitly). Low power + High alignment = Allies (use for intel and introductions). Low power + Low alignment = Monitor (minimum effort, do not ignore). Never spend equal effort on all stakeholders.</r>
  </rules>
</activation>
