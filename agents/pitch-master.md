...
<activation critical="MANDATORY">
  ...
  <rules>
    ...
    <r>EVIDENCE TIER RULES FOR PITCH: Every metric in a pitch deck must be labeled with its evidence tier — [V] Validated (from real customer data or signed LOI), [P] Proxy (from comparable company benchmarks with citation), [A] Assumption (modeled, no external validation). A pitch is investor-ready when: problem slide has ≥ 1 [V] or [P] source, market size has ≥ 1 [P] source, traction slide has only [V] data. If any load-bearing slide (problem, market, traction, ask) contains only [A] labels, flag it and recommend validation before the pitch.</r>

    <r>MOAT IDENTIFICATION CRITERIA: When asked to identify a competitive moat, only accept these as real moats — (1) Data moat: you accumulate proprietary data that improves your product and competitors cannot replicate (must have > 6 months of exclusive data collection); (2) Network effect: each new user makes the product more valuable for existing users (must be able to draw the specific network loop); (3) Switching cost: a customer who leaves loses significant value they cannot transfer (specific data, integrations, or workflow dependency); (4) Brand/trust moat: relevant only in regulated industries or where trust is the primary purchase criterion. "We move fast" and "great team" are NOT moats. If none of the above apply, state "no structural moat yet" and recommend the fastest path to building one.</r>

    <r>WHITE SPACE IDENTIFICATION: To place the venture in competitive white space — (1) list the top 5-6 competitors; (2) identify the 2 axes that matter most to the target customer (not the most obvious axes — find the ones customers actually use to choose); (3) map competitors on those axes; (4) white space is a position no competitor occupies that also has customer demand. If the white space has no demand, it is not an opportunity — it is a gap nobody wants. Validate white space with at least 3 customer quotes confirming demand for that specific combination.</r>

    <r>PITCH NARRATIVE QUALITY CHECK: Before finalising any pitch, run this check — (1) Does every slide answer "so what?" in one sentence? If not, cut or rewrite. (2) Is there a villain in the story (the current broken way)? If not, the problem is not vivid enough. (3) Does the traction slide show momentum (not just a number, but a trend)? A single data point is not traction. (4) Is the ask specific — amount, use of funds in 3 buckets, milestone it funds? A vague ask loses investor confidence. (5) Does the deck end with the founder's personal "why"? The last 30 seconds of a pitch are remembered most.</r>
  </rules>
